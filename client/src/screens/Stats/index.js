...
const StatsMain = () => {
  ...
  const [percentPerformedInMonthShow, setPercentPerformedInMonthShow] = useState(0)
  ...
  async function addStats() {
    ...
    let days_in_month = 32 - new Date(dateNow.getFullYear(), dateNow.getMonth(), 32).getDate();
      ...
      if (element.type === 1) {
        for (let variable in element.dates) {
          if (Number(variable.slice(3, 5)) === thisMonth) {
            ...
            } else if (dayOfPerformed <= lastDayOfSecondWeek && dayOfPerformed >= firstDayOfSecondWeek) {
              positiveForWeeks[1] += 1;
            } else if (dayOfPerformed <= lastDayOfThirdWeek && dayOfPerformed >= firstDayOfThirdWeek) {
              positiveForWeeks[2] += 1;
            } else if (dayOfPerformed <= lastDayOfFourthWeek && dayOfPerformed >= firstDayOfFourthWeek) {
              positiveForWeeks[3] += 1;
            } else if (dayOfPerformed <= lastDayOfFifthWeek && dayOfPerformed >= firstDayOfFifthWeek) {
              positiveForWeeks[4] += 1;
            } else if (dayOfPerformed <= lastDayOfSixthWeek && dayOfPerformed >= firstDayOfSixthWeek) {
    ...
    let allPerformed = allPerformedPositiveNum + countsBadDays - allPerformedNegativeNum + 1; // ПЛЮС 1 - КОСТЫЛЬ ЕСТЬ КОСЯК С ДАТАМИ
    setAllPerformedShow(allPerformed);
    let percentOfPositive = allPerformedPositiveNum / thisDay * 100;
    let percentOfNegative = (countsBadDays + 1 - allPerformedNegativeNum) / thisDay * 100; // ПЛЮС 1 - КОСТЫЛЬ ЕСТЬ КОСЯК С ДАТАМИ
